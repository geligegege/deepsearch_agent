...
def list_sql_tables()-> str:
    ...
    try:
        #确保资源使用安全，使用with语句自动管理连接和游标的生命周期
        with connect(**config) as connection:
            with connection.cursor() as cursor:
                cursor.execute("SHOW TABLES;")
                tables = cursor.fetchall()#获取所有表的结果，返回一个列表，每个元素是一个元组
                if tables:
    ...
    try:
        with connect(**config) as connection:
            with connection.cursor() as cursor:
                ...
                description = cursor.description #获取列信息，包含列名和数据类型等,如果表不存在或者没有列信息，description可能为None
                if not description:
                    return f"表{table_name}没有列信息，无法查询数据！"
                columns= [col[0] for col in description] #获取列名列表
                rows = cursor.fetchall() #获取表数据，返回一个列表，每个元素是一个元组，元组中的值对应列信息
                result=[",".join(map(str, row)) for row in rows] #第一行是列信息，列之间使用,分割
